...
**Chores are definitions; completions are work records.** `Chore` (kind `ASSIGNED` | `OPEN` | `PER_UNIT`, optional recurrence, day-key `startDay`, `missedPolicy`) is what parents create/edit and carries no lifecycle status. `ChoreCompletion` is a child's work on one occurrence: `PENDING → COMPLETED → APPROVED` (reject returns to `PENDING`; rejected per-unit logs are deleted). A completion snapshots its reward at creation, so definition edits never change in-flight or approved work. Approval runs in a Prisma `$transaction`: a conditional status flip plus an `EARNED` transaction — nothing is ever spawned.

**Occurrences are projected, never materialized ahead of action.** Reads never write: `GET /api/chores` and `/api/chores/calendar` merge completion rows with occurrences computed from each definition's schedule (`server/src/lib/projection.ts`). A completion row is created only by a user action (complete, claim, log-units, parent mark-done). The `@@unique([choreId, occurrenceKey])` constraint is the concurrency guard — claim/complete races resolve to one winner and a 409. Dates are household-local `"YYYY-MM-DD"` day keys throughout (`server/src/lib/tz.ts`); per-unit logs use a null key (unlimited), one-offs use `"once"`. Chores are archived (`archivedAt`), never deleted.

**Allowance balance is derived, never stored.** `GET /api/allowance/:childId` sums all of a child's transactions on the fly. Transactions are either `EARNED` (from approved completions, linked via `completionId`) or `ADJUSTMENT` (manual parent correction, can be negative).
